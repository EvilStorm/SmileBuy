...
public class RangeSeekBar<T extends Number> extends ImageView {
    ...
    private NinePatch backLine;
    private NinePatch activeLine;
    ...
    private void init(Context context, AttributeSet attrs) {
        float barHeight;
        int thumbNormal = R.drawable.gauge_dot;
        int thumbPressed = R.drawable.gauge_dot;
        int thumbDisabled = R.drawable.gauge_dot;
        int thumbShadowColor;
        int defaultShadowColor = Color.argb(75, 0, 0, 0);
        int defaultShadowYOffset = Utility.dpToPx(context, 2);
        int defaultShadowXOffset = Utility.dpToPx(context, 0);
        int defaultShadowBlur = Utility.dpToPx(context, 2);

        if (attrs == null) {
            setRangeToDefaultValues();
            mInternalPad = Utility.dpToPx(context, INITIAL_PADDING_IN_DP);
            barHeight = Utility.dpToPx(context, LINE_HEIGHT_IN_DP);
            ...
            try {
                ...
                if (normalDrawable != null) {
                    thumbImage = Utility.drawableToBitmap(normalDrawable);
                }
                Drawable disabledDrawable = a.getDrawable(R.styleable.RangeSeekBar_thumbDisabled);
                if (disabledDrawable != null) {
                    thumbDisabledImage = Utility.drawableToBitmap(disabledDrawable);
                }
                Drawable pressedDrawable = a.getDrawable(R.styleable.RangeSeekBar_thumbPressed);
                if (pressedDrawable != null) {
                    thumbPressedImage = Utility.drawableToBitmap(pressedDrawable);
                }
        ...
        Bitmap bitmap = BitmapFactory.decodeResource(context.getResources(), R.drawable.gauge_bar2);
        backLine = new NinePatch(bitmap, bitmap.getNinePatchChunk(), "");
        bitmap = BitmapFactory.decodeResource(context.getResources(), R.drawable.gauge_bar);
        activeLine = new NinePatch(bitmap, bitmap.getNinePatchChunk(), "");
        //backLine = (NinePatchDrawable) context.getResources().getDrawable(R.drawable.gauge_bar);
        //activeLine = (NinePatchDrawable) context.getResources().getDrawable(R.drawable.gauge_bar2);
        barHeight = backLine.getHeight();
        ...
        mTextSize = Utility.dpToPx(context, DEFAULT_TEXT_SIZE_IN_DP);
        mDistanceToTop = Utility.dpToPx(context, DEFAULT_TEXT_DISTANCE_TO_TOP_IN_DP);
        mTextOffset = !mShowTextAboveThumbs ? 0 : this.mTextSize + Utility.dpToPx(context,
    ...
    public double getScreenMinValue() {
        return normalizedToScreen(normalizedMinValue);
    }
    public double getScreenMaxValue() {
    ...
    public void setSelectedMinValue(T value) {
        ...
        if (listener != null) {
            listener.onRangeSeekBarValuesChanged(this, getSelectedMinValue(), getSelectedMaxValue(), normalizedToScreen(normalizedMinValue), normalizedToScreen(normalizedMaxValue));
        }
    }
    ...
    protected synchronized void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int height = thumbImage.getHeight()
                + (!mShowTextAboveThumbs ? 0 : Utility.dpToPx(getContext(), HEIGHT_IN_DP))
...
//            String maxLabel = getContext().getString(R.string.demo_max_label);
        ...
        if (mShowTextAboveThumbs && (mActivateOnDefaultValues || !selectedValuesAreDefault)) {
            ...
            int offset = Utility.dpToPx(getContext(), TEXT_LATERAL_PADDING_IN_DP);
    ...
    public interface OnRangeSeekBarChangeListener<T> {
        ...
        void onRangeSeekBarValuesChanged(RangeSeekBar<?> bar, T minValue, T maxValue, double minX, double maxX);
    }
